...
export class Pjax {
  ...
  click = (event, container, options) => {
    ...
    let clickEvent = new CustomEvent('pjax:click', { bubbles: true, cancelable: true });
    link.dispatchEvent(clickEvent, [opts]);

    if (!clickEvent.defaultPrevented) {
      this.call(opts);
      event.preventDefault();

      let clickedEvent = new CustomEvent('pjax:clicked', { bubbles: true, cancelable: true });
      link.dispatchEvent(clickedEvent, [opts]);
    };
  }
  ...
  _extractContainer = (data, response, options) => {
    ...
    if (fullDocument) {
      ...
      head = body = this._parseHTML(data);
    }
